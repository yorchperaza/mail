...
function logerr(string $msg): void { fwrite(STDERR, $msg.(str_ends_with($msg, "\n") ? '' : "\n")); }
...
// ---- get Redis client (container -> env -> build) ----
$redis = null;
...
// ---- helper closures (define BEFORE use) ----
...
$xClaim = function (string $s, string $g, string $c, int $minIdle, array $ids) use ($redis) {
    try {
        ...
        /** @var PredisClient $redis */
        return $redis->executeRaw(array_merge(['XCLAIM',$s,$g,$c,(string)$minIdle], $ids));
    } catch (\Throwable) { return null; }
};
$xAdd = function (string $s, array $fields) use ($redis) {
    if ($redis instanceof Redis) return $redis->xAdd($s, '*', $fields);
    /** @var PredisClient $redis */
    $cmd = ['XADD',$s,'*']; foreach ($fields as $k=>$v){ $cmd[]=(string)$k; $cmd[]=(string)$v; }
    return $redis->executeRaw($cmd);
};

// ---- ensure group at 0 and drain backlog (use helpers defined above) ----
$ensureGroupAtZero = function(string $s, string $g) use ($redis) {
    try {
        if ($redis instanceof Redis) {
            $redis->xGroup('CREATE', $s, $g, '0', true);
        } else {
            /** @var PredisClient $redis */
            $redis->executeRaw(['XGROUP','CREATE',$s,$g,'0','MKSTREAM']);
        }
        logerr("[worker] XGROUP CREATE {$s} {$g} 0 (mkstream)");
    } catch (\Throwable $e) {
        try {
            if ($redis instanceof Redis) { $redis->xGroup('SETID', $s, $g, '0'); }
            else { $redis->executeRaw(['XGROUP','SETID',$s,$g,'0']); }
...
while (true) {
    try {
        ...
        foreach ($msgs[$stream] as $entryId => $fields) {
            ...
            if (!is_array($payload) || !isset($payload['message_id'])) {
                $xAck($stream, $group, [$entryId]); // prevent PEL clog
                continue;
            }
